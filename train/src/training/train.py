...
from training.data import make_supervised_conv_data_module
...
def train():
    ...
    if "Qwen2.5" in model_args.model_id:
        ...
        print(f"loading model from {model_args.model_id}")
            ...
            if 'lm_head' in name or 'embed_token' in name:
                if hasattr(module, 'weight'):
                    if training_args.bf16 and module.weight.dtype == torch.float32:
                        module = module.to(torch.bfloat16)
    rank0_print("Making sft data...")
    data_module = make_supervised_conv_data_module(model_id=model_args.model_id,
                                              processor=processor,
                                              data_args=data_args)
