...
def get_image_info(image_path, min_pixel, max_pixel):
    ...
    # 这一步有些多余，因为 processor load进来已经带了 min，max pixels，处理结果是一样的
    messages = [
        {"role": "user", 
         "content": [
             {
...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if pixel_key and grid_key:
            # breakpoint()
            # rank0_print(len(all_pixel_values))
...
# TODO: 合并 conv_dataset 和 grounding_dataset，用 is_grounding 标记区分
def make_supervised_conv_data_module(model_id, processor, data_args):
...
def make_supervised_grounding_data_module(model_id, processor, data_args):
    """Make dataset and collator for supervised fine-tuning."""
    grounding_dataset = UI_Grounding_Dataset(
        data_path=data_args.data_path, processor=processor, data_args=data_args, model_id=model_id
    )
    data_collator = DataCollatorForSupervisedDataset(pad_token_id=processor.tokenizer.pad_token_id)
    if data_args.eval_data_path is not None:
        eval_dataset = UI_Grounding_Dataset(
            data_path=data_args.eval_data_path, processor=processor, data_args=data_args, model_id=model_id
        )
    return dict(train_dataset=grounding_dataset,
                eval_dataset=eval_dataset,
                data_collator=data_collator)